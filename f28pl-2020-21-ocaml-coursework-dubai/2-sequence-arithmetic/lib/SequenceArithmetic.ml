 ...
 *)
 let rec seq_add : int_seq -> int_seq -> int_seq =
  (*recursive function that takes xs and ys and matches them to 3 different scenarios (separated by the "| symbol), 
    x and y are then assigned as the first elements in their respective list and then added together and the lists are returned which gives us the answer*)
  fun xs ys -> match xs, ys with | [],[] -> [] 
  |  _, [] -> [] | [],_ -> [] | (x::remaining), (y::left) -> 
...
let rec seq_mult : int_seq -> int_seq -> int_seq =
   (*recursive function that takes xs and ys and matches them to 3 different scenarios (separated by the "| symbol), 
    x and y are then assigned as the first elements in their respective list and then multiplied together and the lists are returned which gives us the answer*)
  fun xs ys -> match xs, ys with | [],[] -> [] 
  |  _, [] -> []
