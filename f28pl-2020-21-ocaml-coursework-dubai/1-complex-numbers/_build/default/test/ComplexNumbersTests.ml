...
let complex_add_test2 _test_ctxt =
  (* TODO *)
  assert_equal (10,9) (complex_add (4,4) (6,5))
  (* these tests show that the function adds the first elements together and the second elements together and outputs them in a tuple-like format *)


(* write a unit test for complex_mult *)

(*All the following mult tests follow the F.O.I.L formula which is what we use for complex multiplication
  however the formula was altered as to somewhat shortcut the process and make sure the first element in the answer is correct*)
...
let mult_commutative =
  QCheck.Test.make ~name:"complex_mult_commutative" ~count:10000
    ...
    (* TODO *)
    (fun (ci1, ci2) -> 
    (*the property tests ensure that the math functions the way it should by making sure that all the error cases are covered*)
      (complex_mult ci1 ci2 = complex_mult ci2 ci1))
...
let add_identity =
  QCheck.Test.make ~name:"complex_add_identity" ~count:10000
    QCheck.(make
              ...
              (complex_number_gen))
    (* TODO *)
    (fun ci1 -> complex_mult (0,0) ci1 = (0,0) && complex_mult ci1 (0,0) = (0,0))
